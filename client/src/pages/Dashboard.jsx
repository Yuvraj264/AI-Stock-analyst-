...
import { FileText, ArrowLeft, ArrowUpRight, ArrowDownRight, RefreshCw, BarChart2, Shield, Eye } from 'lucide-react';
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-md overflow-hidden">
            ...
            <div className="p-6">
              ...
              {activeTab === 'sentiment' && (
                <div className="space-y-6">
                  <div>
                    <h4 className="font-bold text-sm text-slate-400 uppercase tracking-wider mb-2">News Catalysts</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
          ...
          <div className="p-6 rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-md">
            ...
            <div className="h-[28rem] overflow-y-auto pr-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300 font-medium space-y-4 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800">
              <div className="prose prose-slate dark:prose-invert max-w-none text-xs">
                {analysis.report ? (
                  analysis.report.split('\n').map((line, idx) => {
                    ...
                    if (line.startsWith('## ')) {
                      return <h2 key={idx} className="text-sm font-bold text-slate-800 dark:text-slate-150 mt-4 mb-1.5 uppercase tracking-wide">{line.replace('## ', '')}</h2>;
                    }
                    if (line.startsWith('### ')) {
                      return <h3 key={idx} className="text-xs font-bold text-emerald-500 dark:text-emerald-400 mt-3 mb-1 uppercase tracking-wide">{line.replace('### ', '')}</h3>;
                    }
                    if (line.startsWith('* ') || line.startsWith('- ')) {
                      return <li key={idx} className="list-disc list-inside ml-2 py-0.5">{line.substring(2)}</li>;
                    }
                    if (line.trim() === '---') {
                      return <hr key={idx} className="border-slate-100 dark:border-slate-800 my-4" />;
                    }
                    return <p key={idx} className="mb-2 text-slate-500 dark:text-slate-400">{line}</p>;
                  })
                ) : (
                  <p>No qualitative summary compiled.</p>
                )}
              </div>
            </div>
          </div>
